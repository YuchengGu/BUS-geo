...
os.environ.setdefault("MPLCONFIGDIR", str(Path(tempfile.gettempdir()) / "matplotlib_cache_geodesic_progress"))
...
INPUT_DIRS: list[str] = []
INPUT_GLOB = str(REPO_ROOT / "breast_path_planning" / "results" / "live_gui_0615_*")
OUTPUT_DIR = SCRIPT_DIR / "geodesic_progress_figure"
...
def main() -> None:
    ...
    if RERUN_OPTIMIZATION or not metrics_path.exists():
        ...
        write_json(metrics_path, {"params": params_dict(GUI_GEODESIC_PARAM_VALUES), "runs": runs})
    else:
...
def compute_progress_metrics(input_dir: Path) -> dict[str, Any]:
    ...
    for record, positions in zip(records, snapshots, strict=True):
        progress.append(float(record["iteration"]) / float(params.max_iterations))
...
def save_individual_panel(
    ...
    for curve in stack:
        axis.plot(grid, curve, color=INDIVIDUAL_COLOR, alpha=0.28, linewidth=0.75)
    axis.plot(grid, stats["mean"], color=MEAN_COLOR, linewidth=2.5)
    baseline = 0.0 if metric_key == "mean_displacement_mm" else 1.0
    axis.axhline(baseline, color=BASELINE_COLOR, linestyle="--", linewidth=0.8, alpha=0.7)
    axis.set_xlim(0.0, 1.0)
    axis.set_xlabel("Normalized optimization progress")
...
def params_dict(params: dict[str, Any]) -> dict[str, float | int | None]:
    return {
