...
class TeleopLoop:
    ...
    def start_surface_positioning(
        ...
        *,
        ...
        self._stop.clear()
        self._thread = threading.Thread(
            target=self._run_surface_positioning,
            kwargs={"controller": controller, "on_sample": on_sample, "on_status": on_status},
            daemon=True,
        )
        self._thread.start()

    def start_surface_recording(
        self,
        *,
        controller,
        recorder: EpisodeRecorder,
        ...
        self._stop.clear()
        self._thread = threading.Thread(
            target=self._run_surface_recording,
            kwargs={"controller": controller, "recorder": recorder, "on_sample": on_sample, "on_status": on_status},
            daemon=True,
        )
        self._thread.start()
    ...
    def _run_surface_positioning(
        ...
        *,
        ...
        try:
            obs = self.devices.get_obs()
            while not self._stop.is_set():
                obs, action, _obs_meta, _action_timing = self.devices.step_surface_cartesian_teleop(controller, obs)
                if on_sample is not None:
                    on_sample({"obs": obs, "action": action})
                time.sleep(self.config.idle_sleep_s)
        except Exception as exc:
            if on_status is not None:
                on_status(f"曲面笛卡尔摆位循环错误: {type(exc).__name__}: {exc}")
    ...
    def _run_surface_recording(
        ...
        *,
        ...
        try:
            ...
            while not self._stop.is_set():
                current_obs = obs
                obs, action, obs_meta, action_timing = self.devices.step_surface_cartesian_teleop(controller, current_obs)
                timestamp = datetime.datetime.now()
                meta = build_time_alignment_meta(
                    sample_index=recorder.sample_index,
                    episode_id=None if recorder.episode_dir is None else recorder.episode_dir.name,
            ...
            if on_status is not None:
                on_status(f"曲面笛卡尔记录循环错误: {type(exc).__name__}: {exc}")
